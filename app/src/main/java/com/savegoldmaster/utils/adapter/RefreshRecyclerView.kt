...
class RefreshRecyclerView(context: Context, attrs: AttributeSet?, defStyleAttr: Int) :
    WrapRecyclerView(context, attrs, defStyleAttr) {
    ...
    private fun restoreRefreshView() {
        ...
        // 回弹到指定位置
        ObjectAnimator.ofFloat(currentTopMargin.toFloat(), finalTopMargin.toFloat()).apply {
            duration = distance.toLong()
            addUpdateListener { animation ->
                setRefreshViewMarginTop((animation.animatedValue as Float).toInt())
            }
            start()
        }
        mCurrentDrag = false
    ...
    fun setRefreshViewMarginTop(marginTop: Int) {
        ...
        val params = mRefreshView?.layoutParams as ViewGroup.MarginLayoutParams
